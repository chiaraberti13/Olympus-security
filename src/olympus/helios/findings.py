...
def export_findings(
    ...
    *,
    ...
    payload = {
        ...
        "alerts": [json.loads(alert.model_dump_json()) for alert in (alerts or [])],
    }
    path.parent.mkdir(parents=True, exist_ok=True)
    path.write_text(json.dumps(payload, indent=2, sort_keys=True), encoding="utf-8")


def load_findings(path: Path) -> tuple[list[Asset], list[Finding], list[Alert]]:
